...
def eval_predictions(net, labels, probs, predictions, cat_names=None, print_results=True):
    ...
    if n_classes==2:   #because label_binarize output is of size (n,1) if the problem is binary
        y_temp=np.zeros(shape=(y_one_hot.shape[0],n_classes))
        y_temp[:,0]=y_one_hot[:,0]
        y_temp[:, 1] = 1 - y_one_hot[:, 0]
        y_one_hot=y_temp
